...
function возврат(i) {
  return {
    ...
    product: {
      sku: String(1100526200 + i), offer_id: `art-${i}`, name: `Товар ${i}`,
...
async function собрать(опции) {
  ...
  try {
    return { строки: await ozonВозвратыСтроки(), запросы };
  } finally {
    сервер.close();
  }
}
...
test('фильтр по датам не шлём вовсе — иначе метод ругается', async () => {
...
test('строка листа собрана по схеме', async () => {
  ...
  assert.equal(первая[4], 'art-0');              // Артикул
