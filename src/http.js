...
export function период(задача) {
  ...
  if (граница && !знак) return { с: граница, по, режим: 'заливка' };

  let с = new Date(по.getTime() - дней * 24 * 3600 * 1000);
  if (знак && знак.getTime() - ЗАПАС < с.getTime()) с = new Date(знак.getTime() - ЗАПАС);
  if (граница && с.getTime() < граница.getTime()) с = граница;
  if (с.getTime() > по.getTime()) {
    // «С даты» в завтрашнем дне: пустой прогон нельзя засчитывать успешным,
    // иначе первичная заливка считается состоявшейся и история не придёт никогда
    throw new Error(`«С даты» ${вТаблицу(с).slice(0, 10)} позже сегодняшнего дня`);
  }
  return { с, по, режим: 'окно' };
}
...
export function описаниеПериода(задача) {
  try {
    ...
    return п.режим === 'заливка'
      ? `первичная заливка с ${д(п.с)}`
      : `период ${д(п.с)} — ${д(п.по)}`;
  } catch {
    return '';
  }
